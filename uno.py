...
def logic_stategia(id_giocatore, carta_centro, lista_carte_valide, len_carte_next_player, get_colore):
...
def fai_mossa(index_turno: int, mazzo_iniziale: Mazzo, mazzi_giocatori: Mano):
    ...
    indice_carta_scelta = logic_stategia(id_giocatore, carta_al_centro, carte_valide, len(mazzi_giocatori[(index_turno+2) % len(giocatori)].player), False)
    ...
    # Carta
        ...
        if carta_scelta.tipo == "+4":
            ...
            nuovo_colore_scelto = logic_stategia(id_giocatore, carta_al_centro, carte_valide, len(mazzi_giocatori[(index_turno+2) % len(giocatori)].player), True)
